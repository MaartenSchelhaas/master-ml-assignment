...
from ml_assignment.model import MLP, MLPConfig
...
if __name__ == "__main__":
    ...
    config = MLPConfig(hidden_sizes=[16, 8], lr=0.01, n_epochs=200, loss="brier", seed=0)
    model = MLP(config)
    history = model.fit(X_tr, y_tr, X_val, y_val)
    print(f"final val Brier score: {history['val_loss'][-1]}")